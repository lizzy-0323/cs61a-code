...
def prune_min(t):
    ...
    def helper(t):
        nonlocal max_val
        if t.is_leaf():
            if t.label > max_val.label:
                max_val = t
            return
        for b in t.branches:
            helper(b)
        t.branches.remove(max_val)
        max_val = t
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            ...
            possibilities.append((code_char+result, cost))
        # Insert
        edits , cost = helper_align(skeleton_idx, code_idx + 1)
        result = "+[" + code_char + "]"
        possibilities.append( (result+ edits, 1 + cost))
        # Delete
...
def num_splits(s, d):
    ...
    # first是当前选择的元素，rest是剩余的列表。n是当前选择的列表
    # 对于每一个子列表，当前元素都可以选择加入或者不加入
    def helper(s, n):
        if len(s) == 0:
