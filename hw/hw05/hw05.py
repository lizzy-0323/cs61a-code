...
def make_withdraw(balance, password):
    ...
    """
    "*** YOUR CODE HERE ***"
    password_attempt_list=[]
    def bank_with_password_protected(amount, password_attempt):
        nonlocal balance,password
        if len(password_attempt_list)>=3:
            return "Too many incorrect attempts. Attempts: "+str(password_attempt_list)
            ...
            password_attempt_list.append(password_attempt)
            return 'Incorrect password'
    return bank_with_password_protected
...
def repeated(t, k):
    ...
    for value in t:
        if value == prev_value:
            ...
            count = 1  # Reset count if the value changes
            prev_value = value
